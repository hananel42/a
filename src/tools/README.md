...
- `searchWikipedia.ts`: Tool execution handler for `search_wikipedia`. Queries the Wikipedia REST API.
- `callAgent.ts`: Tool execution handler for `call_agent`. Triggers recursive sub-agent conversation loops with `agent-id`, `prompt`, and optional `resume-id`, returning structured `{ status, id, msg }` JSON.
